...
#
def profile_view():
    profile = db(db.auth_user.id==auth.user_id).select(db.auth_user.id, db.auth_user.first_name, db.auth_user.last_name, db.auth_user.image).first()
    table = db(db.info.user_email==auth.user.email).select(db.info.skills, db.info.available_times).first()
    print(table)
